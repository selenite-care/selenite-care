...
type MembershipAnalyticsSummary = {
  total: number;
  active: number;
  pending: number;
  expired: number;
...
type MembershipAnalyticsResponse = {
...
const EMPTY_SUMMARY: MembershipAnalyticsSummary = {
  total: 0,
  active: 0,
  pending: 0,
  expired: 0,
...
export default function MembershipAnalyticsWidget() {
  ...
  const [summary, setSummary] = useState<MembershipAnalyticsSummary>(EMPTY_SUMMARY);
  ...
  useEffect(() => {
    ...
    async function loadAnalytics() {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/analytics/memberships?${params.toString()}`, {
          cache: "no-store",
        ...
        if (!response.ok) {
          throw new Error(data?.error ?? "Unable to load membership analytics.");
        }
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Unable to load membership analytics.",
        );
      } finally {
        if (!controller.signal.aborted) {
          setIsLoading(false);
            ...
            return (
              <button
                ...
                className={`inline-flex h-9 items-center justify-center gap-2 rounded-full px-3 text-sm font-medium transition-colors ${
                  isActive
                    ? "bg-[#2B2B2B] text-[#F8F5F0] dark:bg-[#8B6914] dark:text-[#F8F5F0]"
            ...
            return (
              <button
                ...
                className={`h-10 shrink-0 rounded-full border px-4 text-sm font-medium transition-colors ${
                  isActive
                    ? "border-[#2B2B2B] bg-[#2B2B2B] text-[#F8F5F0] dark:border-[#8B6914] dark:bg-[#8B6914] dark:text-[#F8F5F0]"
                    : "border-[#D8C7B5] bg-[#F8F5F0] text-[#8C7967] hover:border-[#8B6914] dark:border-[#3D3530] dark:bg-[#1A1814] dark:text-[#8A7D75]"
                }`}
              >
                {tab.label}
              </button>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
          <label className="block">
            ...
            <input
              ...
              className="mt-2 h-11 w-full rounded-md border border-[#D8C7B5] bg-[#F8F5F0] px-3 text-sm text-[#2B2B2B] outline-none focus:border-[#8B6914] dark:border-[#3D3530] dark:bg-[#1A1814] dark:text-[#F0EDE8]"
            />
          </label>
          ...
          <button
            ...
            className={`inline-flex h-11 items-center justify-center rounded-md px-5 text-sm font-semibold transition-colors ${
              customDatesApplied
                ...
                : "bg-[#8B6914] text-[#F8F5F0] hover:bg-[#9E7A1A]"
            }`}
          >
          ...
          <ResponsiveContainer width="100%" height={300}>
            {chartType === "bar" ? (
              <BarChart data={chartData}>
                ...
                />
                <Bar dataKey="memberships" fill="#8B6914" radius={[6, 6, 0, 0]} />
              </BarChart>
            ) : (
              <LineChart data={chartData}>
                ...
                <Line
                  ...
                  stroke="#8B6914"
                  strokeWidth={3}
                  dot={{ fill: "#8B6914", strokeWidth: 0, r: 4 }}
