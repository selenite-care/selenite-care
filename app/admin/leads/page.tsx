...
export default function AdminLeadsPage() {
  ...
  const [intentChartData, setIntentChartData] = useState<IntentChartPoint[]>([]);
  const [intentMetricsTotal, setIntentMetricsTotal] = useState(0);
  const [searchQuery, setSearchQuery] = useState("");
  const [qualityFilter, setQualityFilter] = useState<"ALL" | LeadQuality>("ALL");
  const [error, setError] = useState("");
  const [intentMetricsError, setIntentMetricsError] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const [isLoadingIntentMetrics, setIsLoadingIntentMetrics] = useState(true);
  ...
  useEffect(() => {
    ...
    async function loadIntentMetrics() {
      try {
        ...
        setIntentMetrics(data?.metrics ?? []);
        setIntentChartData(data?.chartData ?? []);
        setIntentMetricsTotal(data?.total ?? 0);
      } catch (loadError) {
        if (!isMounted) {
          return;
        }

        setIntentMetricsError(
        ...
        if (isMounted) {
          setIsLoadingIntentMetrics(false);
        }
      }
    }

    void loadIntentMetrics();
  ...
  return (
    <section className="min-h-screen bg-[#F8F5F0] px-6 py-10 dark:bg-[#1A1814]">
      ...
      <section className="mt-8 rounded-2xl border border-[#EADDCD] bg-white p-5 shadow-sm dark:border-[#3D3530] dark:bg-[#242220]">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="text-sm text-[#884F38] dark:text-[#8A7D75]">
            {intentMetricsTotal} onboarding submissions in the last 30 days
          </p>
        </div>

        {isLoadingIntentMetrics ? (
