...
export default function NewAdminBlogPostPage() {
  ...
  const slugPreview = useMemo(() => slugify(title) || "post-title", [title]);
  ...
  async function savePost(
    ...
    try {
      const response = await fetch(
        savedSlug ? `/api/blog/${savedSlug}` : "/api/blog",
        {
          method: savedSlug ? "PATCH" : "POST",
          headers: {
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-sm font-medium text-[#8C7967] dark:text-[#8A7D75]">
            <Link href="/admin/blog" className="hover:text-[#B87B68]">
              Blog Posts
            </Link>{" "}
            / New Post
          </p>
